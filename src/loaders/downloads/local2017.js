...
/** Build a generic CSV → sheet pass-through, replacing vote_share with % values */
function buildCsvSheet(wb, sheetName, rows) {
  ...
  for (const row of rows) {
    sheet.addRow(keys.map(k => {
      const v = row[k];
      if (k === "vote_share" && v != null && v !== "" && v !== "NA") return safePct(v);
      return v ?? "";
    }));
  }
  finishSheet(sheet, keys.map(k => Math.min(40, Math.max(10, k.length + 4))));
}
...
function buildMetadataSheet(wb, election, sub, generatedAt) {
  ...
  const rows = [
    ...
    ["Data Source", (election.sources?.[0]?.name?.en ?? "Central Election Commission of Georgia (cesko.ge)")],
    ["Citation (APA)", `Sichinava, D. (${generatedAt.getFullYear()}). Results of the ${(election.name?.en ?? election.id) + subLabel}. Comprehensive Election Data Archive of Georgia (CEDAG). https://electionsdata.ge/${election.id}`],
...
export async function generateLocal2017Downloads({ generatedAt = new Date() } = {}) {
  ...
  for (const sub of subElections(election)) {
    const entry = await generateBundle(election, sub, generatedAt);
    results.push(entry);
  }
  return results;
}
